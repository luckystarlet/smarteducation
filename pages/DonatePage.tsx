...
const ImpactGoalBar: React.FC<ImpactGoalProps> = ({ title, current, goal, description, color }) => (
  <div>
    <div className="flex justify-between items-end mb-1">
      <span className="font-medium text-gray-700">{title}</span>
      <span className="text-sm font-bold text-blue-700">{current} / {goal}</span>
    </div>
...
const ContributionCard: React.FC<ContributionCardProps> = ({ amount, description }) => (
  <div className="bg-blue-50 p-4 rounded-lg flex flex-col justify-center text-center">
    <p>
      <span className="font-bold text-lg block text-blue-800">{amount}</span>
      {description}
    </p>
  </div>
);
...
const DonatePage: React.FC = () => {
  ...
  const impactGoals: ImpactGoalProps[] = [
    {
  ...
  const contributionCards: ContributionCardProps[] = [
    {
      amount: 'Ksh 1,000',
      description: "provides a term's supply of sanitary pads for two girls."
    },
    {
      amount: 'Ksh 2,500',
      description: 'supplies school materials for a student for one year.'
    },
    {
      amount: 'Ksh 5,000',
      description: "sponsors a student's participation in mentorship workshops."
    }
  ];
              ...
              <div className={`border-2 p-6 rounded-lg transition ${
                ...
                <div className="space-y-2 mb-4">
                  <div className="flex items-center justify-between bg-white p-3 rounded">
                    ...
                    >
                      {copiedField === 'mpesa-phone' ? <Check className="text-green-600" size={20} /> : <Copy className="text-gray-600" size={20} />}
                    </button>
                  </div>
                  <div className="flex items-center justify-between bg-white p-3 rounded">
                    <div>
                    ...
                    <button
                      ...
                      className="p-2 hover:bg-gray-100 rounded transition"
                    >
                      {copiedField === 'mpesa-name' ? <Check className="text-green-600" size={20} /> : <Copy className="text-gray-600" size={20} />}
                    </button>
                  </div>
                </div>
                <button
                ...
                <div className="space-y-2 mb-4 text-left">
                  ...
                  <div className="flex items-center justify-between bg-white p-3 rounded">
                    ...
                    <button
                      ...
                      className="p-2 hover:bg-gray-100 rounded transition"
                    >
                      {copiedField === 'bank-account' ? <Check className="text-green-600" size={20} /> : <Copy className="text-gray-600" size={20} />}
                    </button>
                  </div>
                </div>
                <button
